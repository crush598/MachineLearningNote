...
class DecisionTree:
    ...
    def is_split_stop(self, num_class_lable, num_samples):
        if self.tree.depth() >= self.max_depth or num_class_lable == 1 or num_samples < self.min_samples_split:
            return True
        return False
    ...
    def fit(self, X, y):
        self.build_tree(X, y)
        # self.tree.show()
        self.tree.save2file("binary tree")
